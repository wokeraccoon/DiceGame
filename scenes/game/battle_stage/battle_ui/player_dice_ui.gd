...
@onready var player_stats: HBoxContainer = %PlayerStats
...
func _on_battle_stage_ready() -> void:
	...
	await get_tree().create_timer(0.25).timeout
	dice_hand_label.text = "[shake rate=20.0 level=5 connected=1][font_size=64]Rolling...[/font_size][/shake]"
	roll_button.show()
	attack_button.show()
	player_stats.show()
	scoring_area.show()
...
func show_dice_combo(dice_combo : DiceCalculator.DiceComboNames) -> void:
	...
	match dice_combo:
		DiceCalculator.DiceComboNames.SINGLES:
			dice_hand_label.text = "[font_size=64]Singles[/font_size]"
		DiceCalculator.DiceComboNames.PAIR:
			dice_hand_label.text = "[font_size=64]Pair[/font_size]"
		DiceCalculator.DiceComboNames.TWO_PAIR:
			dice_hand_label.text = "[font_size=64]Two Pair[/font_size]"
		DiceCalculator.DiceComboNames.THREE_OF_A_KIND:
			dice_hand_label.text = "[wave amp=50 freq=5][font_size=64]Three of a kind[/font_size][/wave]"
		DiceCalculator.DiceComboNames.FOUR_OF_A_KIND:
			dice_hand_label.text = "[wave amp=50 freq=5][font_size=64]Four of a kind[/font_size][/wave]"
		DiceCalculator.DiceComboNames.FULL_HOUSE:
			dice_hand_label.text = "[wave amp=50 freq=5][font_size=64]Full house[/font_size][/wave]"
		DiceCalculator.DiceComboNames.STRAIGHT:
			dice_hand_label.text = "[wave amp=50 freq=5][font_size=64]Straight[/font_size][/wave]"
		DiceCalculator.DiceComboNames.YATCH:
			dice_hand_label.text = "[rainbow sat=0.5][wave amp=100 freq=5][font_size=64]Yatch![/font_size][/wave][/rainbow]"
