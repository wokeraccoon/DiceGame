...
var dice_combos_scores : Dictionary[DiceComboNames,int] = {
	DiceComboNames.SINGLES : 5,
	DiceComboNames.PAIR : 10,
...
func check_for_dice_combos(dice_rolled : Dictionary) -> DiceComboNames:
	...
	if dice_sides_ammounts.size() == 1 and dice_rolled.size() == 5:
	...
	if dice_sides_ammounts.size() == 2 and dice_rolled.size() == 5:
